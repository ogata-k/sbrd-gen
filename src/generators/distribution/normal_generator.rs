...
use crate::{DataValue, DataValueMap, GeneratorBuilder, GeneratorType, Nullable, SbrdReal};
            ...
            Self: Sized,
    {
        ...
        let (mean, std_dev): (SbrdReal, SbrdReal) = match parameters{
            ...
            Some(parameters) => {
                let _mean = parameters
                    .get(Self::MEAN)
                    .map(|v| v.to_parse_string().parse::<SbrdReal>().map_err(|e| {
                ...
                if _std_dev < 0.0 {
                    return Err(CompileError::InvalidValue(format!("std_dev {} is less than 0.0", _std_dev)));
                }
        ...
        Ok(Self {
            ...
            distribution: Normal::new(mean, std_dev)
                .map_err(|e| CompileError::FailBuildDistribution("Normal".to_string(), e.to_string()))?
        })
    }
